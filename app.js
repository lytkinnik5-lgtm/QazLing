/* ============================================================================
   QazLing — single-file app (vanilla JS, hash router)
...
(function () {
  ...
  function seedDB() {
    const db = {
      users: [
        { id: "u_admin", name: "Админ", email: "admin@qazling.kz", pass: "admin", role: "admin", createdAt: Date.now() - 86400000 * 30, profile: null,
  ...
  window.QazLing = { api, SEED, get DB() { return DB; } };
  ...
  /* ============================================================= UI: SHELL */
  /* ============================================================= I18N */
  let lang = localStorage.getItem("qazling.lang") || "ru";
  const I18N = {
    ru: {
  ...
  function setLang(l) { if (l !== "ru" && l !== "kk") return; lang = l; localStorage.setItem("qazling.lang", l); render(); }
    ...
    const aside = `
      <aside class="hidden lg:flex lg:flex-col lg:w-72 lg:fixed lg:inset-y-0 border-r border-line bg-white/80 backdrop-blur-xl">
        <div class="px-5 py-5 flex items-center gap-3 border-b border-line/60">
          ...
          <div>
            <div class="font-black text-lg leading-none tracking-tight grad-text">QazLing</div>
    ...
    const top = `
      <header class="lg:hidden sticky top-0 z-30 bg-white/90 backdrop-blur-xl border-b border-line px-4 h-14 flex items-center justify-between">
        <div class="flex items-center gap-2.5">
          ...
          <span class="font-black tracking-tight grad-text text-lg">QazLing</span>
        </div>
        <div class="flex items-center gap-2.5">
          ${langSwitcher("")}
  ...
  route("auth", () => {
    ...
    const draw = () => {
      root.innerHTML = `
        ...
        <div class="ornament relative hidden lg:flex flex-col justify-between grad-bg text-white p-12 overflow-hidden">
          ...
          <div class="relative flex items-center gap-3">
            ...
            <span class="font-black text-2xl tracking-tight">QazLing</span>
          </div>
          ...
          <p class="relative text-white/40 text-sm">© QazLing · Тіл — ұлттың жаны</p>
        </div>
        ...
        <div class="flex items-center justify-center p-6 sm:p-10 min-h-screen lg:min-h-0">
          <div class="w-full max-w-sm anim-up">
            <div class="lg:hidden flex items-center gap-2.5 mb-8 justify-center">
              ...
              <span class="font-black text-xl tracking-tight grad-text">QazLing</span>
            </div>
            ...
            <div class="mt-5 rounded-xl bg-gold-soft border border-gold/30 p-3 text-sm">
              ...
              <button data-demo="admin" class="block text-muted hover:text-brand">Админ: admin@qazling.kz / admin</button>
            </div>
          </div>
        </div>`;
      ...
      $$("[data-demo]", root).forEach((b) => b.addEventListener("click", () => {
        const map = { student: ["aru@mail.kz", "1234"], admin: ["admin@qazling.kz", "admin"] };
    ...
    wrap.innerHTML = `
      <aside class="hidden lg:flex lg:flex-col lg:w-72 lg:fixed lg:inset-y-0 border-r border-line bg-ink text-white">
        <div class="px-6 py-6 flex items-center gap-3">
          ...
          <div><div class="font-extrabold text-lg leading-none">QazLing</div><div class="text-xs text-white/50 mt-1">Админ-панель</div></div>
        </div>
  ...
  route("admin", () => {
    ...
    const content = `
      <div class="space-y-6">
        <div><h1 class="text-2xl font-extrabold">Обзор платформы</h1><p class="text-muted mt-1">Ключевые метрики QazLing</p></div>
